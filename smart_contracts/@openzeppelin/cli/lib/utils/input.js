...
function parseArg(input, type) {
    ...
    // Address: just validate them
        ...
        if (!upgrades_1.ZWeb3.isAddress(input)) {
    ...
    // Bytes: same, just check they are a valid hex
        ...
        if (!upgrades_1.ZWeb3.isHex(input)) {
    ...
    // Warn if we see a type we don't recognise, but return it as is
        ...
        upgrades_1.Loggy.noSpin.warn(__filename, 'parseArg', `Unknown argument ${type} (skipping input validation)`);
        return input;
    }
}
